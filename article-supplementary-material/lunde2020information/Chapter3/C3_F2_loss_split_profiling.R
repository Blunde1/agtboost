...
p1 <- df %>%
    ggplot() + 
    geom_line(aes(u, tr_l), colour="black", alpha=0.7, size=0.4, linetype = "solid") + 
    geom_line(aes(u, te_l), colour="#56B4E9", alpha=0.8, size=0.4, linetype="solid") +
    geom_hline(yintercept = unique(df$l0), linetype="longdash", colour="#009E73", size=1) + 
    #geom_text(data=data.frame( x = 0.4, y= unique(df$l0)), map=aes(x=x, y=y), label = "Asymptotic value", vjust=-1) + 
    ylab("Loss") + 
    xlab(TeX('$ u = \\frac{i}{n}')) + 
    #xlab(TeX('$ u = p(x_j \\leq s ) $')) + 
    ggtitle("Loss profiling") +
    theme_minimal()
p1

p2 <- df %>%
    ggplot() + 
    geom_line(aes(u, n/C*(te_l-tr_l)), colour="black", alpha=0.7, size=0.4, linetype="solid") + 
    ylab("L.h. side of (22)") + 
    #ylab(TeX('$ \\frac{n}{\\hat{C}_{stump}} (E_0\\[l_{te}\\]-l_{tr}) $')) + 
    xlab(TeX('$ u = \\frac{i}{n}')) + 
    ggtitle("Loss optimism") +
    theme_minimal()
p2

p3 <- df %>%
    ggplot() +
    geom_line(aes(tau, n/C*(te_l-tr_l)), colour="black", alpha=0.7, size=0.4, linetype="solid") + 
    ylab(TeX('$ 1+S(\\tau) $')) + 
...
if(F)
{
    ...
    #pdf("split_seq_to_cir3.pdf", width=8, height=3.5, paper="special")
    grid.arrange(p1,p2,p3, ncol=3)
